    ...
    for (uint32_t i = 0; i < stl->stats.number_of_facets; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            if (edge_a.p1 != edge_b.p1 || edge_a.p2 != edge_b.p2) {
                ...
                stl_write_facet(stl, reinterpret_cast<char*>("first facet"), i);
                stl_write_facet(stl, reinterpret_cast<char*>("second facet"), neighbor);
            }
        }
    }
}
...
static inline void rotate_point_2d(float &x, float &y, const double c, const double s) {
    double xold = x;
    double yold = y;
    x = static_cast<float>(c * xold - s * yold);
    y = static_cast<float>(s * xold + c * yold);
}
